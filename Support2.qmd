...
data_missing10 = data_missing10 |> 
  mutate(cluster3 = as.integer(ifelse(cluster == 1, 1, 0)))

data_joined = data_joined |> 
  mutate(cluster1 = as.integer(ifelse(cluster == 1, 1, 0)))
```
...
data_long <- data_joined %>%
  select(cluster1, num.co, charges, sps, aps) %>%
...
ggplot(data_long, aes(x = factor(cluster3), y = valore, fill = factor(cluster3))) +
  ...
  labs(x = "Cluster1", y = "Value", 
       title = "Cluster 1 analysis") +
  theme_minimal() +
