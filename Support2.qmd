...
## INCOMPLETO
(FINIRE DI GESTIRE I NA NEI MODELLI SUCCESSIVI)
...
## 👎 PCA + Kmeans
```{r, warning=FALSE, }
vars_to_remove <- c( "alb",
  "adls", "bili", "pafi", "ph", "prg2m", "edu", "prg6m")
...
```
algoritmo kmeans sulle prime 6 PC
```{r}
set.seed(6)  
km_res <- kmeans(pca$x[, 1:6], centers = 2, nstart = 25)
...
## ? KMEANS (CON GMM feature selection)
...
load_pc1 |> arrange(desc(loading)) |> head(4)
load_pc1 |> arrange(loading) |> head(4)
```
...
## Tandem Analysis su tutte le variabili numeriche, le prime due dimensioni spiegano il 50 % della varianza totale. Silhouhette suggerisce 2 clusters mentre Gap statistics ne identifica solo 1. Valutare questa discordanza
...
## Confronto numeriche/categoriche
```{r}
# --- 1. K-means numerico su 3 variabili ---
set.seed(123)
...
# --- 2. MCA + K-means su variabili categoriche ---
cat_data <- data |> select(where(is.factor))
...
# --- 3. Costruzione dataframe con entrambe le assegnazioni ---
...
# --- 4. Matrice di confusione tra le due colonne ---
...
```
```{r}
ggplot(tot_data_scaled, aes(x = death, y = ..prop.., group = 1)) +
...
# --- 7. Fit finale e valutazione su test ---
