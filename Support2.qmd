...
res_cluspca <- num_data_scaled |> 
  cluspca(ndim = 3, nclus = 2, method = "RKM")
...
## 👎 FKM (DA FARE tuning per ndim e nclus)

```{r}
res_cluspca <- num_data_scaled |> 
  cluspca(ndim = 3, nclus = 2, method = "FKM")
...
df_clusters <- data.frame(
  id = 1:nrow(data_scaled),  # indice osservazioni
...
tuning
Grid Search prova tutte le combinazioni possibili di iperparametri in una griglia predefinita, mentre Bayesian Optimization non prova tutte le combinazioni: costruisce un modello probabilistico dello spazio dei parametri
```{r}
set.seed(123)
...
Dopo il clustering fare Decision Tree, con variabile target i cluster
...
mca_dataframe <- data.frame(
  ...
  Dim1    = res_mca$ind$coord[, 1],
  Dim2    = res_mca$ind$coord[, 2],
  Dim3    = res_mca$ind$coord[, 3]
)
